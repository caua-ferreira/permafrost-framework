...
class PermafrostMaster:
    ...
    def _build_app(self):
        ...
        def heartbeat(worker_id: str, data: dict):
            with self._lock:
        ...
        def task_done(job_id: str, task_id: str, data: dict):
            with self._lock:
                job = self.jobs.get(job_id)
                if not job: return {"ok": False}
        ...
        def task_failed(job_id: str, task_id: str, data: dict):
            with self._lock:
                job = self.jobs.get(job_id)
                if not job: return {"ok": False}
                for task in job.tasks:
                    if task.task_id == task_id:
                        ...
                        if task.retries < self.MAX_RETRIES:
                            ...
                            job.error   = f"Task {task_id} falhou após {self.MAX_RETRIES} tentativas: {task.error}"
    ...
    def _plan_and_schedule(self, job_id: str):
        """Divide o job em tasks e enfileira para scheduling."""
        ...
        # Ler o arquivo para saber quantas linhas tem e criar as tasks
        try:
            ...
            if ext == '.csv':
                total = sum(1 for _ in open(source)) - 1  # header
                ...
                total = chunk   # fallback: 1 task

            # Criar tasks — 1 task por chunk
            tasks = []
    ...
    def _schedule_pending_tasks(self):
        """Distribui tasks da fila para workers idle."""
        while not self._task_queue.empty():
            with self._lock:
                idle = [w for w in self.workers.values() if w.status == "idle"]
                if not idle: break
                worker = idle[0]   # round-robin simples
                try:
            ...
            # Enviar task para o worker (fora do lock)
            self._dispatch_task(worker, job, task)

    def _dispatch_task(self, worker: WorkerInfo, job: Job, task: Task):
        """Envia uma task para um worker via HTTP."""
        payload = {
    ...
    def _check_job_completion(self, job: Job):
        """Verifica se todas as tasks concluíram e atualiza o job."""
        if not job.tasks: return
        done    = [t for t in job.tasks if t.status == TaskStatus.DONE]
        failed  = [t for t in job.tasks if t.status == TaskStatus.FAILED]
        if failed:
            job.status = JobStatus.FAILED; return
...
class PermafrostWorker:
    ...
    Cada task = 1 chunk de dados → freeze → notifica Master.
    """

    HEARTBEAT_INTERVAL = 5   # segundos

    def __init__(self, master_url: str, host="127.0.0.1",
                 port=8800, worker_id: str = None):
    ...
    def _build_app(self):
        ...
        @app.post("/execute")
        def execute(data: dict, background_tasks: BackgroundTasks):
            background_tasks.add_task(self._run_task, data)
            return {"accepted": True, "task_id": data['task_id']}

        return app

    def _run_task(self, data: dict):
        """Executa o freeze de um chunk e reporta ao Master."""
        ...
        try:
            ...
            ext = os.path.splitext(source)[1].lower()
            if ext == '.csv':
                df_chunk = pd.read_csv(source, skiprows=range(1, start+1),
                                       nrows=n_rows)
            elif ext in ('.jsonl', '.ndjson'):
            ...
            # Comprimir o chunk
            from permafrost_v4 import freeze, CODEC_LZMA2, CODEC_ZSTD, QUANT_NONE, QUANT_MEDIUM
            codec_map = {'lzma2': CODEC_LZMA2, 'zstd': CODEC_ZSTD}
            codec = codec_map.get(cfg.get('codec','lzma2'), CODEC_LZMA2)
            quant = cfg.get('quant', 0)

            # Output: arquivo de chunk separado (serão mesclados pelo Master)
            chunk_out = f"{output}.chunk_{chunk_idx:04d}.permafrost"
            metrics = freeze(df_chunk, chunk_out,
    ...
    def _report_done(self, master_url, job_id, task_id, result):
        try:
    ...
    def _report_failed(self, master_url, job_id, task_id, error):
        try:
            with httpx.Client(timeout=10) as c:
    ...
    def _start_heartbeat(self, master_url):
        def hb():
            while True:
    ...
    def register(self, master_url: str = None):
        """Registra este worker no Master."""
    ...
    def run(self, auto_register=True, **kwargs):
        if auto_register:
            # Tentar registrar após o servidor iniciar
            def delayed_register():
                time.sleep(1.5)
                try: self.register()
                except Exception as e: print(f"Register falhou: {e}")
...
class PermafrostClient:
    ...
    def freeze(self, source_path: str, output_path: str = None,
        ...
        """Submete um job de freeze para o cluster. Retorna job_id."""
        payload = {
    ...
    def wait(self, job_id: str, poll_interval: float = 0.5,
        ...
        """Aguarda um job completar. Retorna o status final."""
        start = time.time()
...
print("permafrost_cluster.py OK")
print("  Classes: PermafrostMaster, PermafrostWorker, PermafrostClient")
